...
{
	public CONST BIOMES = ["PLAINS" => BiomeIds::PLAINS, "DESERT" => BiomeIds::DESERT, "MOUNTAINS" => BiomeIds::EXTREME_HILLS, "FOREST" => BiomeIds::FOREST, "TAIGA" => BiomeIds::TAIGA, "SWAMP" => BiomeIds::SWAMPLAND, "NETHER" => BiomeIds::HELL, "HELL" => BiomeIds::HELL, "ICE_PLAINS" => BiomeIds::ICE_PLAINS, "END" => BiomeIds::THE_END];
	...
	public function execute(CommandSender $sender, array $args) : bool {
		if(count($args) === 0) {
			...
			$sender->sendMessage(MyPlot::getPrefix() . $this->translateString("biome.possible", [$biomes]));
			return true;
		}
		...
		if(is_numeric($biome)) {
			...
			if($biome > 27 or $biome < 0) {
				$sender->sendMessage(MyPlot::getPrefix() . TextFormat::RED . $this->translateString("biome.invalid"));
				$biomes = implode(", ", array_keys(self::BIOMES));
				$sender->sendMessage(MyPlot::getPrefix() . TextFormat::RED . $this->translateString("biome.possible", [$biomes]));
				return true;
			}
